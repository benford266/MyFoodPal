...
Adapted from original RecipeGenerator class
"""
import httpx
import json
import aiohttp
...
class RecipeGeneratorService:
    """Service for generating recipes using LM Studio/Ollama"""
    
    def __init__(self, 
                 lm_studio_url: str = None, 
                 model: str = None):
        self.lm_studio_url = lm_studio_url or settings.LM_STUDIO_BASE_URL
    ...
    async def generate_single_recipe(self, 
                                   ...
                                   existing_ingredients: List[str] = None, 
                                   progress_callback: Optional[Callable] = None, 
                                   total_recipes: int = 5, 
                                   serving_size: int = 4, 
                                   used_carbs: List[str] = None, 
                                   user_id: int = None, 
                                   db_session = None, 
                                   must_use_ingredients: List[str] = None) -> Dict[str, Any]:
        ...
        # Track used elements
        if not hasattr(self, 'used_cooking_inspirations'): 
            self.used_cooking_inspirations = []
...
Generate a complete, authentic {selected_cuisine} recipe now:"""
    ...
    async def generate_recipes(self, 
                             ...
                             user_id: int = None, 
                             db_session = None, 
                             must_use_ingredients: List[str] = None) -> List[Dict[str, Any]]:
        """Generate specified number of recipes with maximum diversity"""
        ...
        for i in range(1, recipe_count + 1):
            # Only pass used carbs for variety, not all ingredients
    ...
    async def generate_recipes_with_images(self, 
                                         liked_foods: List[str], 
                                         disliked_foods: List[str], 
                                         recipe_count: int = 5, 
                                         serving_size: int = 4, 
        ...
        """Generate recipes and optionally generate images"""
        ...
        # Generate images if requested and enabled
        if generate_images and settings.ENABLE_IMAGE_GENERATION and recipes:
            ...
            try:
                ...
                if valid_recipes:
                    ...
                    for idx, (recipe_index, recipe) in enumerate(valid_recipes):
                        ...
                        try:
                            image_path = await image_service.generate_recipe_image(
                                recipe['name'],
                    ...
                    # Summary
                    successful_images = sum(1 for recipe in recipes if recipe.get('image_path'))
                ...
                print("⚠️ Image generation service not available, skipping image generation")
            except Exception as e:
